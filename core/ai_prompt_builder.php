...
class AiPromptBuilder
{
    ...
    public static function appendTourContext(string $basePrompt, string $tourContext): string
    {
        ...
        return $base . "\n\n"
            ...
            . "--------------------------------------------------\n\n"
            . "請嚴格遵守（回覆給客人時）：\n"
            . "1. 僅引用上述搜尋結果中實際出現的行程名稱、最近出團日期、價格；不得捏造或改寫。\n"
            . "2. 以固定清單回覆：每筆必須有 1. 2. 3. 編號；第一行為完整行程標題；避免「想玩○○？」等分類式小標。\n"
            . "3. 「" . GeminiTourContextBuilder::DEPARTURE_DATE_LABEL . "」僅用 MM/DD（例 06/01、06/10），不要顯示年份；不得顯示今天以前的過期出團日；同一商品最多 " . GeminiTourContextBuilder::MAX_DEPARTURE_DATES_DISPLAY . " 個出團日；超過時第 " . GeminiTourContextBuilder::MAX_DEPARTURE_DATES_DISPLAY . " 個日期後須保留尾端「" . GeminiTourContextBuilder::MORE_DEPARTURE_DATES_SUFFIX . "」；日期後勿加「可參考」「可選擇」等字。\n"
            . "4. 金額行請用「直售價：」開頭，格式盡量與參考一致（必要時句尾加「起」）。\n"
            . "5. 必須附上參考中的「" . GeminiTourContextBuilder::SEARCH_URL_LABEL . "」URL（若存在）；不得省略或改寫網址。\n"
            . "6. 若參考中已將同名行程多個出發日合併，請維持單筆列點與「、」分隔的日期；若參考最近出團含「" . GeminiTourContextBuilder::MORE_DEPARTURE_DATES_SUFFIX . "」須逐字保留。\n"
            . "7. 每兩筆行程之間必須保留參考中的「" . GeminiTourContextBuilder::LINE_TOUR_ITEM_SEPARATOR . "」分隔線（單獨一行，僅連續「=」組成；逐字一致）；最後一筆行程與「" . GeminiTourContextBuilder::SEARCH_URL_LABEL . "」之間不要重複空行，也不要再多加任何分隔線。\n"
            . "8. 參考中的「" . GeminiTourContextBuilder::DETAIL_URL_LABEL . "」URL 必須逐字保留，不得改寫或替換。\n"
            . "9. 參考中的「行程表：」區塊（單筆為「行程表：https://...」；多筆為編號 URL 列表，僅 URL、不含名稱標籤）必須逐字保留；若參考無行程表則不要自行新增。\n"
